    ...
    request.pg.client.query(select, [ request.payload.email ], function(err, result) {
      ...
      if (!err && result.rowCount === 1) { // email exists, lets check password
        ...
        reply.view('index', {
          ...
          values : { email: help.validator.escape(request.payload.email) }
        }).code(400);
      }
      request.pg.done(); // return the pg connection to the "pool"
      return; // always return
    }); // END request.pg.client.query

    // return reply.view('success', {
    //   name   : 'Friend', // "Hi Friend!" (until we add name to reg form)
